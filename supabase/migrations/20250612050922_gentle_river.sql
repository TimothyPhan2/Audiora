/*
  # Remove Premium Tier from Subscription System
  
  This migration removes the 'premium' tier from the subscription_tier_enum,
  keeping only 'free' and 'pro' tiers. All existing 'premium' users are 
  migrated to 'pro' tier to maintain their access level.
  
  ## Changes Made:
  1. Update existing 'premium' data to 'pro'
  2. Drop dependent views and functions temporarily
  3. Replace enum type with new version (free, pro only)
  4. Update all table columns to use new enum
  5. Recreate all dependent objects with updated schema
...
-- =====================================================
-- 2. DROP DEPENDENT OBJECTS TEMPORARILY
-- =====================================================
...
-- Drop functions that depend on the subscription_tier_enum
...
-- =====================================================
-- 8. DROP OLD ENUM TYPE (NOW SAFE)
-- =====================================================

-- Drop the old enum type (now safe since no objects reference it)
DROP TYPE public.subscription_tier_enum_old;

-- =====================================================
-- 9. RECREATE HELPER FUNCTIONS WITH NEW ENUM
-- =====================================================

-- Recreate user_has_premium_access function to check for 'pro' instead of 'premium'
...
-- Recreate get_user_subscription_tier function with new enum return type
CREATE OR REPLACE FUNCTION get_user_subscription_tier()
RETURNS public.subscription_tier_enum AS $$
DECLARE
...
/*
  ...
  SELECT get_user_subscription_tier();
  ...
  -- 7. Verify no old enum type exists
  SELECT typname FROM pg_type WHERE typname LIKE '%subscription_tier_enum%';
  -- Should only show 'subscription_tier_enum', not 'subscription_tier_enum_old'
*/
...
/*
  ...
  ✅ Dropped dependent views and functions temporarily to avoid conflicts
  ...
  ✅ Dropped old enum type cleanly (no dependencies)
  ✅ Recreated user_has_premium_access() function to check for 'pro'
  ✅ Recreated get_user_subscription_tier() function with new enum
  ...
  - All views and functions recreated and functional
  ...
  6. Test all helper functions work correctly
*/