...
async function uploadAudioToStorage(audioBuffer, filename) {
  const { data, error } = await supabase.storage.from('pronunciation-files').upload(filename, audioBuffer, {
  ...
  // Get public URL
  const { data: urlData } = supabase.storage.from('pronunciation-files').getPublicUrl(filename);
  return urlData.publicUrl;
}
// Helper function to save exercise to database
async function saveExerciseToDatabase(songId, exercise, audioUrl, difficulty, language) {
  const { data, error } = await supabase.from('pronunciation_exercises').insert({
    song_id: songId,
...
function createPronunciationExercisePrompt(songTitle, artist, lyrics, language, difficulty, userVocabulary) {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!voiceId) {
      return new Response(`Unsupported language: ${language}`, {
        status: 400,
        headers: corsHeaders
      });
    }
    // Check if pronunciation exercises already exist (caching)
    const { data: existingExercises } = await supabase.from('pronunciation_exercises').select('*').eq('song_id', songId).eq('difficulty_level', difficulty);
    ...
    const prompt = createPronunciationExercisePrompt(songData.title, songData.artist, songLyrics, language, difficulty, userVocabulary);
